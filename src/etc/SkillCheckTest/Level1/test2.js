...
const photo = [["may", "kein", "kain", "radi"], ["may", "kein", "brin", "dyka"]];
console.log(solution(name, yearning, photo));

function solution(name, yearning, photo) {
    const map = new Map();
    name.forEach((name, idx) => {
        map.set(name, yearning[idx]);
    });

    return photo.map(peoples => peoples.reduce((acc, name) => acc + (map.get(name) ?? 0), 0));
}

/**
||는 Falsy 값(undefined, null, 0, false, "", NaN)을 기준으로 대체값을 반환합니다.
하지만 여기서 대체값 0은 undefined일 때만 적용되어야 하며, 0은 유효한 값으로 처리해야 합니다.
??는 null 또는 undefined일 때만 대체값을 반환하므로, 0 같은 값이 그대로 유지됩니다.
 */
